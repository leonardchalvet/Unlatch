...
$app->get('/{lg}/{uid}', function ($request, $response, $args) use ($app, $prismic) {
    ...
    $document = NULL;
    $nType = 0;
    $arrayTypes = ['home', 'solutions', 'features', 'p404']; // UPDATE NAME OF CUSTOM TYPE HERE (only if exist in CONTENT)
    $arrayView  = ['home', 'solutions', 'feature', '404']; // NAME IN "VIEWS" FOLDER, ALWAYS SAME POSITION BETWEEN "arrayTypes" & "arrayView"
    foreach ($arrayTypes as $type) {
